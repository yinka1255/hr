	...
	class ReduxFramework_slides {
		...
		public function render() {
			$defaults    = array(
				...
				'content_title'  => __( 'Slide', 'consultix' )
			);
			$this->field = wp_parse_args( $this->field, $defaults );
			echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr( sprintf( __( 'New %s', 'consultix' ), $this->field[ 'content_title' ] ) ) . '">';
			$x           = 0;
			...
			if ( isset( $this->value ) && is_array( $this->value ) && !empty( $this->value ) ) {
				...
				foreach ( $slides as $slide ) {
					...
					echo '<span class="button media_upload_button" id="add_' . $x . '">' . __( 'Upload', 'consultix' ) . '</span>';
					$hide = '';
					if ( empty( $slide[ 'image' ] ) || '' == $slide[ 'image' ] ) {
						$hide = ' hide';
					}
					echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __( 'Remove', 'consultix' ) . '</span>';
					echo '</div>' . "\n";
					...
					$placeholder = ( isset( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'title' ] ) : __( 'Title', 'consultix' );
					...
					if ( $this->field[ 'show' ][ 'description' ] ) {
						$placeholder = ( isset( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'description' ] ) : __( 'Description', 'consultix' );
					...
					$placeholder = ( isset( $this->field[ 'placeholder' ][ 'btitle' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'btitle' ] ) : __( 'Button Title', 'consultix' );
					...
					$placeholder = ( isset( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'url' ] ) : __( 'URL', 'consultix' );
					...
					echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . __( 'Delete', 'consultix' ) . '</a></li>';
					echo '</ul></div></fieldset></div>';
					$x ++;
				}
			}
			if ( 0 == $x ) {
				echo '<div class="redux-slides-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-slides-header">' . esc_attr( sprintf( __( 'New %s', 'consultix' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';
				$hide = ' hide';
				echo '<div class="screenshot' . $hide . '">';
				...
				echo '<span class="button media_upload_button" id="add_' . $x . '">' . __( 'Upload', 'consultix' ) . '</span>';
				echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __( 'Remove', 'consultix' ) . '</span>';
				echo '</div>' . "\n";
				echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="redux-slides-list">';
				if ( $this->field[ 'show' ][ 'title' ] ) {
					$title_type = "text";
			...
			echo '</div><a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field[ 'name_suffix' ] . '">' . sprintf( __( 'Add %s', 'consultix' ), $this->field[ 'content_title' ] ) . '</a><br/>';
		}
