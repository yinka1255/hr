...
class ReduxFramework_typography extends ThemegumReduxFramework{
    ...
    if ($this->field['font-family'] == true):
          ...
          echo '<div id="'.$this->field['id'].'" class="redux-typography-container" data-id="'.$this->field['id'].'" data-units="'.$unit.'">';
              ...
              echo '<select data-placeholder="'.esc_attr__('Font family','petro').'" class="redux-typography redux-typography-family '.$this->field['class'].'" id="'.$this->field['id'].'-family" data-id="'.$this->field['id'].'" data-value="'.$fontFamily[0].'">';
              ...
              if ( isset($this->field['update_weekly']) && $this->field['update_weekly'] == true && $this->field['google'] == true && !empty( $this->args['google_api_key'] ) ) {
                  ...
                  if( file_exists( ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.html' )) {
                    // Keep the fonts updated weekly
                    $weekback = strtotime( date('jS F Y', time() + (60 * 60 * 24 * -7) ) );
                    $last_updated = filemtime( ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.html' );
                    if ( $last_updated < $weekback ) {
                      unlink( ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.html' );
                      unlink( ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.json' );
                    }                 
                  }
              }
              ...
              if ( $this->field['google'] == true && !empty( $this->args['google_api_key'] ) ) {
                echo '<optgroup label="'.esc_html__('Standard Fonts', 'petro').'">';  
              }
              ...
              if ($this->field['google'] == true ) {
                  ...
                  } elseif( file_exists( ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.html' )) {

                    $googleHTML = $wp_filesystem->get_contents( ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.html' );
            ...
            if ($this->field['font-style'] == true || $this->field['font-weight'] == true):
                echo '<div class="select_wrapper typography-style" original-title="'.esc_attr__('Font style','petro').'">';
              $style = $this->value['font-weight'].$this->value['font-style'];
                ...
                echo '<select data-placeholder="'.esc_attr__('Style','petro').'" class="redux-typography redux-typography-style select'.$this->field['class'].'" original-title="'.esc_attr__('Font style','petro').'" id="'. $this->field['id'].'_style" data-id="'.$this->field['id'].'" data-value="'.$style.'">';
            ...
            if ($this->field['subsets'] == true && $this->field['google'] == true):
                echo '<div class="select_wrapper typography-script tooltip" original-title="'.esc_attr__('Font subsets','petro').'">';
                echo '<select data-placeholder="'.esc_attr__('Subsets','petro').'" class="redux-typography redux-typography-subsets'.$this->field['class'].'" original-title="'.esc_attr__('Font script','petro').'"  id="'.$this->field['id'].'-subsets" name="'.$this->args['opt_name'].'['.$this->field['id'].'][subsets]" data-value="'.$this->value['subsets'].'" data-id="'.$this->field['id'].'" >';
            ...
            if ($this->field['font-family'] == true && $this->field['google'] == true) { 
              // Set a flag so we know to set a header style or not
                ...
                if ($this->field['font-backup'] == true) {
                  ...
                  echo '<select data-placeholder="'.esc_attr__('Backup Font Family','petro').'" name="'.$this->args['opt_name'].'['.$this->field['id'].'][font-backup]" class="redux-typography redux-typography-family-backup '.$this->field['class'].'" id="'.$this->field['id'].'-family-backup" data-id="'.$this->field['id'].'" data-value="'.$this->value['font-backup'].'">';
                  echo '<option data-google="false" data-details="" value=""></option>';
                  foreach ($this->field['fonts'] as $i=>$family) {
            ...
            if ($this->field['color'] == true):
                ...
                echo '<input data-default-color="'.$default.'" class="redux-color redux-typography-color'.$this->field['class'].'" original-title="'.esc_attr__('Font color','petro').'" id="'.$this->field['id'].'-color" name="'.$this->args['opt_name'].'['.$this->field['id'].'][color]" type="text" value="'. $this->value['color'] .'" data-id="'.$this->field['id'].'" />';
            endif;
    ...
    function enqueue(){
        ...
        wp_enqueue_script(
            ...
            ThemegumReduxFramework::$_url . 'inc/fields/color/field_color.js', 
        ...
        wp_enqueue_script(
            ...
            ThemegumReduxFramework::$_url.'inc/fields/typography/field_typography.js',
        ...
        wp_enqueue_style(
            ...
            ThemegumReduxFramework::$_url.'inc/fields/typography/field_typography.css',
            "",
            true
        );
    ...
    function output() {
      ...
      if ( !empty( $fonts ) && filter_var($this->parent->args['output'], FILTER_VALIDATE_BOOLEAN) ) {
        wp_enqueue_style( 'redux-google-fonts', $this->makeGoogleWebfontLink( $fonts ), '', $version );
      }
      if ( !empty($outCSS ) ) {
    ...
    */
    function getGoogleArray() {
        ...
        if( !file_exists( ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.json' ) ) {
          ...
          if ($result['response']['code'] == 200) {
              ...
              if ( !empty( $this->parent->googleArray ) ) {
                  $wp_filesystem->put_contents(
                      ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.json',
    ...
    function getGoogleFonts() {
        ...
        $gfonts = '<optgroup label="'.esc_html__('Google Webfonts', 'petro').'">';
        ...
        $wp_filesystem->put_contents(
            ThemegumReduxFramework::$_dir.'inc/fields/typography/googlefonts.html',
            $gfonts,
            FS_CHMOD_FILE // predefined mode settings for WP files
        );
    }
