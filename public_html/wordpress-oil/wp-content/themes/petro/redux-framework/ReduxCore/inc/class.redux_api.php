    ...
    if ( ! class_exists( 'ThemegumRedux' ) ) {
        ...
        class ThemegumRedux {
            ...
            public static function load() {
                add_action( 'after_setup_theme', array( 'ThemegumRedux', 'createRedux' ) );
                add_action( 'init', array( 'ThemegumRedux', 'createRedux' ) );
                add_action( 'switch_theme', array( 'ThemegumRedux', 'createRedux' ) );
            }

            public static function init( $opt_name = "" ) {
                if ( ! empty( $opt_name ) ) {
                    self::loadRedux( $opt_name );
                    remove_action( 'setup_theme', array( 'ThemegumRedux', 'createRedux' ) );
                }
            }

            public static function loadExtensions( $ThemegumReduxFramework ) {
                if ( $instanceExtensions = self::getExtensions( $ThemegumReduxFramework->args['opt_name'], "" ) ) {
                    foreach ( $instanceExtensions as $name => $extension ) {
                        if ( ! class_exists( $extension['class'] ) ) {
                            // In case you wanted override your override, hah.
                            $extension['path'] = apply_filters( 'redux/extension/' . $ThemegumReduxFramework->args['opt_name'] . '/' . $name, $extension['path'] );
                            if ( file_exists( $extension['path'] ) ) {
                                require_once $extension['path'];
                            }
                        }
                        if ( ! isset( $ThemegumReduxFramework->extensions[ $name ] ) ) {
                            if ( class_exists( $extension['class'] ) ) {
                                $ThemegumReduxFramework->extensions[ $name ] = new $extension['class']( $ThemegumReduxFramework );
                            } else {
            ...
            public static function extensionPath( $extension, $folder = true ) {
                if ( ! isset( ThemegumRedux::$extensions[ $extension ] ) ) {
                    return;
                }
                $path = end( ThemegumRedux::$extensions[ $extension ] );
                if ( ! $folder ) {
                    return $path;
                }
            ...
            public static function loadRedux( $opt_name = "" ) {
                ...
                $check = ThemegumReduxFrameworkInstances::get_instance( $opt_name );
                ...
                $redux                   = new ThemegumReduxFramework( $sections, $args );
                $redux->apiHasRun        = 1;
            ...
            public static function checkExtensionClassFile( $opt_name, $name = "", $class_file = "", $instance = "" ) {
                if ( file_exists( $class_file ) ) {
                    ...
                    $version                   = ThemegumRedux_Helpers::get_template_version( $class_file );
            ...
            public static function getAllExtensions() {
                $redux = ThemegumReduxFrameworkInstances::get_all_instances();
                foreach ( $redux as $instance ) {
                    ...
                    if ( ! empty( $instance['extensions'] ) ) {
                        ...
                        ThemegumRedux::getInstanceExtensions( $instance['args']['opt_name'], $instance );
                    }
                }
            }
            ...
            public static function getInstanceExtensions( $opt_name, $instance = array() ) {
                ...
                if ( empty( $instance ) ) {
                    $instance = ThemegumReduxFrameworkInstances::get_instance( $opt_name );
                }
                ...
                foreach ( $instance->extensions as $name => $extension ) {
                    ...
                    if ( isset( $extension->extension_dir ) ) {
                        ThemegumRedux::setExtensions( $opt_name, str_replace( $name, '', $extension->extension_dir ) );

                    } else if ( isset( $extension->_extension_dir ) ) {
                        ThemegumRedux::setExtensions( $opt_name, str_replace( $name, '', $extension->_extension_dir ) );
                    }
                }
            }
            ...
            public static function getExtensions( $opt_name = "", $key = "" ) {
                ...
                if ( empty( $opt_name ) ) {
                    ThemegumRedux::getAllExtensions();
                    if ( empty( $key ) ) {
                    ...
                    if ( empty( self::$uses_extensions[ $opt_name ] ) ) {
                        ThemegumRedux::getInstanceExtensions( $opt_name );
                    }
