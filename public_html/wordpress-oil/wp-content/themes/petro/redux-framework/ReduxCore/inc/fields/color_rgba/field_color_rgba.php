       ...
       */
        ...
        public function render() {
            ...
            if ('' == $this->value['color'] || 'transparent' == $this->value['color']) {
                ...
                $color = ThemegumRedux_Helpers::hex2rgba($this->value['color'], $this->value['alpha']);
            }            

            if ($this->value['rgba'] == ''){
                $this->value['rgba'] = ThemegumRedux_Helpers::hex2rgba($this->value['color'], $this->value['alpha']);
            }
        ...
        public function enqueue() {
            ...
            if (!wp_script_is ( 'redux-field-color-rgba-js' )) {
                wp_enqueue_script(
                    ...
                    ThemegumReduxFramework::$_url . 'inc/fields/color_rgba/field_color_rgba' . ThemegumRedux_Functions::isMin() . '.js',
            ...
            if ($this->parent->args['dev_mode']) {
                if (!wp_style_is ( 'redux-field-color-rgba-css' )) {
                    wp_enqueue_style(
                        ...
                        ThemegumReduxFramework::$_url . 'inc/fields/color_rgba/field_color_rgba.css',
                        array(),
                        time(),
                        'all'
                    );
            ...
            if (is_array($this->value)) {
                ...
                foreach($this->value as $id => $val) {
                    ...
                    if ($id == 'alpha') {
                        ...
                        $rgba = !empty($val) ? $val : '';
                        $rgba = ThemegumRedux_Helpers::hex2rgba($color, $alpha);
                    }
                }
        ...
        public function output() {
            if (!empty($this->value)) {
                ...
                if ( ! empty( $this->field['output'] ) && is_array( $this->field['output'] ) ) {
                    $css = ThemegumRedux_Functions::parseCSS( $this->field['output'], $style, $color_val );
                    $this->parent->outputCSS .= $css;
                }

                if ( ! empty( $this->field['compiler'] ) && is_array( $this->field['compiler'] ) ) {
                    $css = ThemegumRedux_Functions::parseCSS( $this->field['compiler'], $style, $color_val );
