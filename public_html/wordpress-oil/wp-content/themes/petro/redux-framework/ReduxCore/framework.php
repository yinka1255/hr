...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ThemegumReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once get_template_directory().'/redux-framework/ReduxCore/inc/class.redux_instances.php';
        require_once get_template_directory().'/redux-framework/ReduxCore/inc/lib.redux_instances.php';
    }

    if ( class_exists( 'ThemegumReduxFrameworkInstances' ) ) {
        add_action( 'petro_redux/init', 'ThemegumReduxFrameworkInstances::get_instance' );
    }

// Don't duplicate me!
    if ( ! class_exists( 'ThemegumReduxFramework' ) ) {
	
		function redux_opt_name(){
			return apply_filters( 'redux_opt_name', "redux_config" );
		}

        // Redux CDN class
        require_once get_template_directory().'/redux-framework/ReduxCore/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once get_template_directory().'/redux-framework/ReduxCore/inc/class.redux_api.php';

        // General helper functions
        require_once get_template_directory().'/redux-framework/ReduxCore/inc/class.redux_helpers.php';

        // General functions
        require_once get_template_directory().'/redux-framework/ReduxCore/inc/class.redux_functions.php';

        require_once get_template_directory().'/redux-framework/ReduxCore/inc/class.redux_filesystem.php';
        ...
        class ThemegumReduxFramework {
            ...
            public static function init() {
                ...
                $dir = ThemegumRedux_Helpers::cleanFilePath( get_template_directory().'/redux-framework/ReduxCore' );

                // Windows-proof constant(name)ts: replace backward by forward slashes. Thanks to: @peterbouwmeester
                self::$_dir           = trailingslashit( $dir );
                self::$wp_content_url = trailingslashit( ThemegumRedux_Helpers::cleanFilePath( ( is_ssl() ? str_replace( 'http://', 'https://', WP_CONTENT_URL ) : WP_CONTENT_URL ) ) );
                self::$_is_plugin = false;

                if ( strpos( ThemegumRedux_Helpers::cleanFilePath( __FILE__ ), ThemegumRedux_Helpers::cleanFilePath( get_template_directory() ) ) !== false ) {
                    $relative_url = str_replace( ThemegumRedux_Helpers::cleanFilePath( get_template_directory() ), '', self::$_dir );
                    self::$_url   = trailingslashit( get_template_directory_uri() . $relative_url );
                } else if ( strpos( ThemegumRedux_Helpers::cleanFilePath( __FILE__ ), ThemegumRedux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false ) {
                    $relative_url = str_replace( ThemegumRedux_Helpers::cleanFilePath( get_stylesheet_directory() ), '', self::$_dir );
                    self::$_url   = trailingslashit( get_stylesheet_directory_uri() . $relative_url );
                } else {
                    $wp_content_dir = trailingslashit( ThemegumRedux_Helpers::cleanFilePath( WP_CONTENT_DIR ) );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
               ...
               // Disregard WP AJAX 'heartbeat'call.  Why waste resources?
                ...
                ThemegumRedux_Functions::$_parent     = $this;
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'petro' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'petro' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'petro' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'petro' );
                }
            ...
            private function set_default_args() {
                ...
                $this->args = array(
                    ...
                    'menu_icon'                 => '',
                    // menu icon
                    'menu_title'                => esc_html__( 'Theme Options', 'petro' ),
                    // menu title/text
                    'page_title'                => esc_html__( 'Theme Options', 'petro' ),
                    // option page title
            ...
            public function _update_check() {
                ...
                if ( ! isset ( $GLOBALS['redux_update_check'] ) ) {
                    ThemegumRedux_Functions::updateCheck( self::$_version );
                    $GLOBALS['redux_update_check'] = 1;
                }
            }

            public function _admin_notices() {
                ThemegumRedux_Admin_Notices::adminNotices( $this->admin_notices );
            }

            public function _dismiss_admin_notice() {
                ThemegumRedux_Admin_Notices::dismissAdminNotice();
            }
            ...
            public function get_options() {
                ...
                if(isset($this->options['site-title'])){
                    $this->options['site-title'] = get_bloginfo('name','raw');
                }


                if(isset($this->options['site-tagline'])){
                    $this->options['site-tagline'] = get_bloginfo('description','raw');
                }

                if(isset($this->options['logo_image'])){
                    ...
                    if(($urls = wp_get_attachment_image_src($image, "full", false))){
                        $logos['url'] = $urls[0];
                        $logos['width'] = $urls[1];
                        $logos['height'] = $urls[2];
                    }

                    if(($urls = wp_get_attachment_image_src($image, "thumbnail", false))){
                        $logos['thumbnail'] = $urls[0];
                    }

                    $this->options['logo_image'] = $logos;

                }


                if(isset($this->options['heading_image'])){
                    $image = get_theme_mod( 'header_image_data' );
                    $logos=array();

                    if($image){
                        $logos = (array) $image;
                        $logos['id'] = $logos['attachment_id'];
                    }

                    $headings = wp_parse_args($logos, array('id'=>'','url'=>'','width'=>'','height'=>'') );

                    $this->options['heading_image'] = $headings;
                }
                /**
            ...
            public function _enqueue() {
                require_once get_template_directory().'/redux-framework/ReduxCore/core/enqueue.php';
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'petro' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'petro' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'petro' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'petro' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'petro' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'petro' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'petro' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            private function _register_extensions() {
                $path    = get_template_directory().'/redux-framework/ReduxCore/inc/extensions/';
            ...
            public function _validate_options( $plugin_options ) {
                ...
                if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
                    ...
                    if ( $plugin_options['import_code'] != '' ) {
                        ...
                        $import = wp_remote_retrieve_body( wp_remote_get( esc_url($plugin_options['import_link']) ) );
                    }
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => esc_html__( 'Invalid security credential, please reload the page and try again.', 'petro' ),
                        'action' => 'reload'
                    ) );
                    die();
                }
                $redux = ThemegumReduxFrameworkInstances::get_instance( $_POST['opt_name'] );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'petro' ) ) );
                    }
                }
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
                        require_once get_template_directory().'/redux-framework/ReduxCore/core/panel.php';
        ...
        /**
         * action 'petro_redux/init'
         *
         * @param null
         */

        do_action( 'petro_redux/init', ThemegumReduxFramework::init() );

        if( file_exists(get_template_directory()."/redux-framework/ReduxCore/settings.php")){
    		require_once(get_template_directory()."/redux-framework/ReduxCore/settings.php");
        }
